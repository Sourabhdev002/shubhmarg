...
import { updateGuidanceRequestStatus, updatePaymentStatus, triggerAiReportGeneration, confirmPaymentManually, rejectPayment } from "./actions";
import { 
  Loader2, 
  Search, 
  Filter, 
...
export default function AdminDashboardClient({ initialRequests }: Props) {
  ...
  const handleConfirmPayment = async (id: string) => {
    setIsUpdating(true);
    setError(null);
    const res = await confirmPaymentManually(id, verificationNote || "Manually confirmed by admin");
    if (res.success) {
      setRequests((prev) => prev.map((r) => (r.id === id ? { ...r, payment_status: "paid" as PaymentStatus, paid_at: new Date().toISOString(), payment_verified_by: "admin" } : r)));
      if (selectedRequest && selectedRequest.id === id) {
        setSelectedRequest({ ...selectedRequest, payment_status: "paid", paid_at: new Date().toISOString(), payment_verified_by: "admin" });
      }
    } else {
      setError(res.error || "Failed to confirm payment");
      setTimeout(() => setError(null), 3000);
    }
    setIsUpdating(false);
  };
  ...
  const handleRejectPayment = async (id: string) => {
    setIsUpdating(true);
    setError(null);
    const res = await rejectPayment(id, verificationNote || "Payment could not be verified");
    if (res.success) {
      setRequests((prev) => prev.map((r) => (r.id === id ? { ...r, payment_status: "payment_failed" as PaymentStatus } : r)));
      if (selectedRequest && selectedRequest.id === id) {
        setSelectedRequest({ ...selectedRequest, payment_status: "payment_failed" });
      }
    } else {
  ...
  const handleGenerateReport = async (id: string) => {
    ...
    const res = await triggerAiReportGeneration(id);

    if (res.success && res.report) {
      setRequests((prev) =>
        prev.map((r) => (r.id === id ? { ...r, notes: res.report || null, status: "completed" as RequestStatus } : r))
      );
      if (selectedRequest && selectedRequest.id === id) {
        setSelectedRequest({
          ...selectedRequest,
          notes: res.report || null,
          status: "completed",
        });
      }
    } else {
      setError(res.error || "Failed to generate report using Gemini.");
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "pending": return "bg-[#d4af37]/10 text-[#a3801f] border-[#d4af37]/30 font-bold";
      case "reviewing": return "bg-blue-50 text-blue-700 border-blue-200 font-bold";
      case "completed": return "bg-teal-50 text-teal-700 border-teal-200 font-bold";
      case "delivered": return "bg-brand-maroon/10 text-brand-maroon border-brand-maroon/30 font-bold";
      case "cancelled": return "bg-red-50 text-red-700 border-red-200 font-bold";
  ...
  const getPaymentStatusColor = (status: string) => {
    switch (status) {
      ...
      case "payment_verification": return "bg-[#d4af37]/10 text-[#a3801f] border-[#d4af37]/30 font-bold";
      case "paid": return "bg-emerald-50 text-emerald-700 border-emerald-200 font-bold";
      case "payment_failed": return "bg-red-50 text-red-700 border-red-200 font-bold";
      case "refunded": return "bg-purple-50 text-purple-700 border-purple-200 font-bold";
      default: return "bg-gray-50 text-gray-700 border-gray-200";
    }
  };
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto flex-wrap">
            <div className="relative">
              ...
              <input 
                ...
                className="pl-9 pr-4 py-2 w-full md:w-64 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#d4af37]/30 focus:border-[#d4af37] outline-none transition-all"
              />
            </div>
            <div className="relative">
              ...
              <select
                ...
                className="pl-9 pr-8 py-2 w-full md:w-48 appearance-none border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-[#d4af37]/30 focus:border-[#d4af37] outline-none transition-all cursor-pointer"
              >
                  ...
                  filteredRequests.map((req) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <button className="text-brand-maroon hover:text-brand-maroon-dark font-medium text-sm opacity-0 group-hover:opacity-100 transition-opacity">
                          View Details &rarr;
                        </button>
                      </td>
                    </tr>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div className="bg-green-50 p-4 rounded-xl border border-green-200">
                ...
                <div className="space-y-3">
                  {selectedRequest.payment_status === "payment_verification" && (
                    <div className="flex gap-2 mb-1">
                      <button
                        type="button"
                        onClick={() => handleConfirmPayment(selectedRequest.id)}
                        disabled={isUpdating}
                        className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold shadow-sm transition-all disabled:opacity-50 cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        disabled={isUpdating}
                        className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2.5 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 rounded-lg text-sm font-bold transition-all disabled:opacity-50 cursor-pointer"
                      >
                  ...
                  <div className="text-sm text-green-800 bg-white/60 p-2 rounded border border-green-100">
                    <div className="flex justify-between mb-1">
                      ...
                      <span>{selectedRequest.payment_amount != null ? `₹${Number(selectedRequest.payment_amount).toFixed(2)}` : 'N/A'}</span>
                    </div>
                    <div className="flex justify-between mb-1">
              ...
              <div>
                ...
                <dl className="grid grid-cols-1 gap-4">
                  ...
                  {/* Automated Guidance Report Block */}
                  <div className="bg-gradient-to-br from-amber-50/70 to-orange-50/60 p-4 border border-amber-200/80 rounded-xl shadow-xs">
                    <div className="flex items-center justify-between mb-3">
                      <dt className="text-sm font-semibold text-amber-950 flex items-center gap-2">
                        <Sparkles className="w-4 h-4 text-amber-600 animate-pulse" />
                        Automated Vedic Report (Gemini 3.7 Flash)
                      </dt>
                      <button
                        type="button"
                        onClick={() => handleGenerateReport(selectedRequest.id)}
                        disabled={isGeneratingReport}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white rounded-lg text-xs font-semibold shadow-xs transition-all disabled:opacity-50 cursor-pointer"
                      >
                        {isGeneratingReport ? (
                          <>
                          ...
                          <>
                            ...
                            {selectedRequest.notes ? "Regenerate Report" : "Generate Report Now"}
                          </>
                        )}
                      </button>
                    </div>
                    ...
                    {selectedRequest.notes ? (
                      <div className="bg-white/90 p-4 border border-amber-100 rounded-lg max-h-72 overflow-y-auto">
                      ...
                      <p className="text-xs text-amber-800/80 italic">
                        No report generated yet. Click &quot;Generate Report Now&quot; to auto-create a complete Vedic reading.
                      </p>
                    )}
                  </div>
                </dl>
